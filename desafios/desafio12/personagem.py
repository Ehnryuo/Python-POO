...
class Personagem(ABC):
    ...
    def atacar(self, alvo, forca = 50):
        if self.vida > 0 and alvo.vida > 0:
            ...
            print(f'[green]{self.nome}[/] ({self.vida}) atacou [red]{alvo.nome}[/] ({self.vida}) com um [yellow]{golpe}')
...
class Mago(Personagem):
    def __init__(self, nome, vida):
        super().__init__(nome, vida)
        self.golpes = ['Bola de Fogo', 'Feixo de Luz', 'Tornado']
    def curar(self):
        fator = random.randint(0, 100)
        self.vida += fator
        print(f'[blue]{self.nome}[/] fez uma mágia de cura e [green] recuperou {fator} pontos de vida[/]')
